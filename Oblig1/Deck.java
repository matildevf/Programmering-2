...
public class Deck {
    ...
    public Collection assign(int n) {
        ...
        for (int i = 0; i < n; i++) {
            randCards.add(cards.get(rand.nextInt(cards.size())));
        }
    ...
    public boolean pokerFlush(ArrayList<Card> list) {
        return list.stream().map(Card::getSuit).collect(Collectors.toSet()).size() == 1;
    }

    public static void main(String[] args) {
        Deck myDeck = new Deck();
        myDeck.makeDeck();
        ArrayList<Card> l = new ArrayList<>(myDeck.assign(5));
